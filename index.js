...
console.log(createMenuItem("Double Cheese Burger", 10, "Breakfast"));
...
console.log(createMenuItem("Chicken Fries", 12, "Dinner"));
console.log(createMenuItem("Strawberry Sundae", 7, "Dessert"));
...
export const burger = {
  ...
  discount: function(customerStatus) {
    ...
    if(customerStatus === 'teacher') {
      discountedPrice = this.price * 0.75;
    } else {
      discountedPrice = this.price * 0.90;
    }
...
console.log(burger.discount("fireman"));
...
function getReviewByIndex(reviewsArray, reviewIndex) {
  let relevantReview = reviewsArray[reviewIndex];
  return `${relevantReview.name} gave the restaurant a ${relevantReview.rating} star review, and their feedback was: ${relevantReview.feedback}`;
}
...
*/
 ...
 function getReviewByRating(reviewsArray, rating) {
    ...
    for(let review of reviewsArray) {
      let selectedRating = Math.floor(review.rating);
